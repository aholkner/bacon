...
public:
    ...
    // 4*4 matrices only
    //
    void translation( const vector< M-1, T >& t,
		typename enable_if< M == N && M == 4 >::type* = 0 );
    void scaling( const vector< M-1, T >& t,
		 typename enable_if< M == N && M == 4 >::type* = 0 );
...
void
matrix< M, N, T >::
translation( const vector< M-1, T >& t,
   typename enable_if< M == N && M == 4 >::type* )
{
	const T _zero = 0.0;
	const T one = 1.0;
